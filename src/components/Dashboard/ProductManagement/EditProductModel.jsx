...
function EditProductModel({ selectProduct, setSelectProduct }) {
    ...
    useEffect(() => {
        ...
        async function getProductById() {
            let productRes = await fetch(
                `https://jsonserver-vercel-api.vercel.app/products/${selectProduct?.id}`
            );
    ...
    const handleSaveProduct = (data) => {
        ...
        let editProduct = {
            ...
            ...data,
            prevPrice:
                Number(data.newPrice) !== Number(currenProduct.newPrice)
                    ? currenProduct.newPrice
                    : currenProduct.prevPrice,
        };
    ...
    const handleReUploadPhoto = async (e) => {
        ...
        if (selectedNewPhoto?.name) {
            ...
            let uploadResult = await axios.post(
                "https://api.cloudinary.com/v1_1/dikortveg/image/upload",
                formData
            );
